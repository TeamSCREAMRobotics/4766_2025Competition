...
public class Controls {
  ...
  private static double MaxSpeed = TunerConstants.kSpeedAt12Volts.in(MetersPerSecond); // kSpeedAt12Volts desired top speed
    ...
    public static void driverControls(Intake Intake, Climber Climber, Elevator Elevator, Manipulator Manipulator, CommandSwerveDrivetrain Drivetrain ) {
    s_Intake = Intake;
    s_Climber = Climber;
    s_Elevator = Elevator;
    ...
    s_Drivetrain.setDefaultCommand(
      ...
      s_Drivetrain.applyRequest(() ->
          drive.withVelocityX(-driverCon.getLeftY() * 0.8) // Drive forward with negative Y (forward)
              .withVelocityY(-driverCon.getLeftX() * 0.8) // Drive left with negative X (left)
              .withRotationalRate(-driverCon.getRightX() * 0.9) // Drive counterclockwise with negative X (left)
      )
  );
  ...
  driverCon.start().and(driverCon.y()).whileTrue(s_Drivetrain.sysIdQuasistatic(Direction.kForward));
  driverCon.start().and(driverCon.x()).whileTrue(s_Drivetrain.sysIdQuasistatic(Direction.kReverse));

  // reset the field-centric heading on left bumper press
  driverCon.leftBumper().onTrue(s_Drivetrain.runOnce(() -> s_Drivetrain.seedFieldCentric()));
  }

  public static void opControls(Intake Intake, Climber Climber, Elevator Elevator, Manipulator Manipulator, CommandSwerveDrivetrain Drivetrain) {
    s_Intake = Intake;
    s_Climber = Climber;
    s_Elevator = Elevator;
    s_Manipulator = Manipulator;
